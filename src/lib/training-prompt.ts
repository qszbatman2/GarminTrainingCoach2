  ...
  return [
    {
      ...
      content: [
        ...
        "不要编造缺失数据，不要做医疗诊断。",
        "允许在 `reasonAnalysis` 和 `weeklyLoadAssessment.reasonAnalysis` 中使用极少量 markdown，仅允许 `**加粗**` 强调关键结论，并允许使用换行分段。",
        ...
        "每周评估的关键结论、标签和小标题必须统一使用规则引擎术语。统一使用“训练不足、训练合理、训练偏多、过度风险”这四个整体结论；训练量和强度结论只允许“不足、偏低、合理、偏高、过高、未知”。禁止使用“进度偏慢”“进度偏快”“练少了”“练多了”等别名。",
    ...
    },
    {
      ...
      content: [
        ...
        "2. `todayAdvice` 可以润色，但不能和规则引擎建议冲突，且必须明确给出今天建议的训练强度和时长；如果规则摘要里已经给出具体训练处方，必须保留这些关键数字和强度词。",
        "3. `reasonAnalysis` 必须控制在 400 字以内。",
        "4. `reasonAnalysis` 必须采用“**结论1**：... 原因：...\\n**结论2**：... 原因：...”的形式，至少输出 2 组结论-原因，其中结论标题必须加粗，且结论2起必须另起一行分段写。",
        ...
        "13. 如果存在用户训练目标，`weeklyLoadAssessment.reasonAnalysis` 必须按目标的各个维度或子目标逐项分析，使用“**目标A**：进度/是否符合预期/原因/关键指标\\n**目标B**：进度/是否符合预期/原因/关键指标”的形式，每个目标标题都要加粗，且从目标B起另起一行分段写。",
        ...
        "16. 没有训练目标时，不要虚构目标，此时按本周训练量、强度、恢复三个维度做简要总结，也要使用加粗小标题并分段换行。",
        "17. 只要写到每周评估结论或小标题，都必须复用规则术语，禁止使用“进度偏慢/进度偏快”等容易误解的表述。",
        "规则引擎最终结论：",
