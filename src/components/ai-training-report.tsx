...
import type { TrainingAnalysisResult, TrainingContext } from "@/lib/training-analysis"
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-500">
            服务端会先压缩最近恢复与训练负荷摘要，再调用火山引擎生成可执行建议，避免直接把原始 JSON 全量喂给模型。
          </p>
        </div>
        <button
          ...
          type="button"
        >
          {loading ? "生成中..." : result ? "重新生成 AI 报告" : "生成 AI 训练报告"}
        </button>
      </div>
